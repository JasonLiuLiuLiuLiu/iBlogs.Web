...
<script>
  ...
  export default {
    ...
    validate({params}) {
      ...
      // 必须是number类型
      return /^\d+$/.test(params.id) && /^\d+$/.test(params.archive)
    },
    components: {right, left, vContent: content},
    async asyncData({store, params}) {
      let pageNum = params.id;
      let pageSize = store.state.options.options.PageSize;
      let response = await archive(params.archive, pageNum, pageSize);
      return {
        data: response.data,
        displayMeta: params.archive,
        displayType: 'archive',
        menuItems: [
          {
            text: '首页',
          ...
          {
            text: dateFormat(new Date(parseInt(params.archive)), 'yyyy年MM月dd日'),
            active: true
          }
        ]
      }
    ...
    methods: {
      ...
      getContentsByNum(pageNum) {
        ...
        archive(this.displayMeta, pageNum, pageSize).then(response => {
