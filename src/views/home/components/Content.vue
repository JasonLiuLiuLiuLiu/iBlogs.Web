<template>
  <div>
    <div class="Card Topstory-noMarginCard Topstory-tabCard">
      <div v-if="displayType=='index'">
        <nav class="TopstoryTabs Topstory-tabs">
          <a class="TopstoryTabs-link Topstory-tabsLink is-active" href="/">
            时间
          </a>
          <a class="TopstoryTabs-link Topstory-tabsLink " href="/hot">
            热度
          </a>
          <a class="TopstoryTabs-link Topstory-tabsLink " href="/random">
            随机
          </a>
        </nav>
      </div>
      <div v-else-if="displayType=='archive'">
        <nav class="TopstoryTabs Topstory-tabs">
          <span class="TagsHeader">
                            <svg class="Zi Zi--Live" fill="currentColor" style="color: rgb(0, 132, 255);"
                                 viewBox="0 0 512 512" width="24" height="24">
                                <g><path
                                  d="m461 145c0-27.123-19.737-49.724-45.602-54.197-4.516-34.267-33.917-60.803-69.398-60.803-5.638 0-11.243.742-16.981 2.257-19.008-20.581-45.257-32.257-73.019-32.257-52.051 0-94.941 39.975-99.583 90.836-25.774 4.548-45.417 27.106-45.417 54.164 0 1.678.08 3.345.229 5h-50.229c-5.523 0-10 4.477-10 10v252c0 2.731 1.158 5.301 2.929 7.071l90 90c1.826 1.826 4.373 2.929 7.071 2.929h150c5.523 0 10-4.477 10-10v-111c0-5.523-4.477-10-10-10s-10 4.477-10 10v101h-130v-80c0-5.523-4.477-10-10-10h-80v-232h46.015c9.289 18.196 28.158 30 48.985 30h125v101c0 5.523 4.477 10 10 10s10-4.477 10-10v-101h95c30.327 0 55-24.673 55-55zm-320 277v55.858l-55.858-55.858zm265-242h-240c-19.224 0-35-15.601-35-35 0-19.299 15.701-35 35-35 5.523 0 10-4.477 10-10 0-44.112 35.888-80 80-80 23.954 0 46.539 10.947 61.962 30.035 2.677 3.313 7.16 4.565 11.166 3.124 5.903-2.126 11.422-3.159 16.872-3.159 27.57 0 50 22.43 50 50 0 5.523 4.477 10 10 10 19.299 0 35 15.701 35 35s-15.701 35-35 35z"/><path
                                  d="m241 276h-120c-5.523 0-10 4.477-10 10s4.477 10 10 10h120c5.523 0 10-4.477 10-10s-4.477-10-10-10z"/>
<path d="m241 336h-120c-5.523 0-10 4.477-10 10s4.477 10 10 10h120c5.523 0 10-4.477 10-10s-4.477-10-10-10z"/><circle
                                    cx="301" cy="346" r="10"/></g>
                            </svg>
            &nbsp {{displayMeta|formatDate}}
            </span>
          <a class="TopstoryTabs-link Topstory-tabsLink is-active"
             href="/archives">
            # {{'归档'}} #
          </a>
        </nav>
      </div>
      <div v-else>
        <nav class="TopstoryTabs Topstory-tabs">
          <a class="TopstoryTabs-link Topstory-tabsLink is-active"
             :href="displayType=='tag'?'/alltags' : '/allcategories'">
            <div v-if="displayType=='tag'">
              <svg class="Zi Zi--Live" fill="currentColor" style="color: rgb(0, 132, 255);" viewBox="0 0 425.7 425.7"
                   width="24" height="24">
                <g>
                  <g>
                    <path
                      d="M167.538,231.652c-2.56-0.512-5.632,1.024-6.144,4.096l-24.064,117.76c-0.512,2.56,1.024,5.632,4.096,6.144c0.512,0,0.512,0,1.024,0c2.56,0,4.608-1.536,5.12-4.096l24.064-117.76C172.146,235.236,170.61,232.164,167.538,231.652z"/>
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M202.354,238.82c-2.56-0.512-5.632,1.024-6.144,4.096l-24.064,117.76c-0.512,2.56,1.024,5.632,4.096,6.144c0.512,0,0.512,0,1.024,0c2.56,0,4.608-1.536,5.12-4.096l24.064-117.76C206.962,241.892,204.914,239.332,202.354,238.82z"/>
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M236.658,245.988c-2.56-0.512-5.632,1.024-6.144,4.096l-24.064,117.76c-0.512,2.56,1.024,5.632,4.096,6.144c0.512,0,0.512,0,1.024,0c2.56,0,4.608-1.536,5.12-4.096l24.064-117.76C241.266,249.06,239.218,246.5,236.658,245.988z"/>
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M412.274,0.74c-2.56-1.536-5.632-0.512-7.168,1.536C367.218,65.764,313.97,108.26,275.57,132.324c-1.536,0-3.584,0-5.12,0l-55.296,4.096l-33.28-6.144c-10.24-2.048-20.48-1.024-30.208,3.072C110.706,107.236,57.97,65.252,20.594,2.788c-1.536-2.56-4.608-3.072-7.168-1.536S10.354,5.86,11.89,8.42c36.864,61.952,88.064,104.448,129.536,131.072c-10.752,7.68-17.92,18.944-20.48,31.744l-35.84,176.64c-5.632,27.648,12.288,55.296,39.936,60.928l77.824,15.872c3.584,0.512,6.656,1.024,10.24,1.024c17.92,0,34.816-9.728,44.032-25.088l32.768-2.56c12.8-1.024,24.576-7.168,33.28-16.896c8.704-9.728,12.8-22.528,11.776-35.328l-12.8-167.936c-1.536-19.456-14.848-35.84-32.256-41.984c38.4-26.112,88.064-67.584,123.904-128C415.346,5.348,414.834,2.276,412.274,0.74z M255.09,382.692c-5.632,20.992-27.136,35.328-49.664,31.232l-77.824-15.872c-22.528-4.608-36.864-26.624-32.256-48.64l35.84-176.64c2.048-10.752,8.704-19.968,17.92-26.112c1.024-0.512,1.536-1.024,2.56-1.536c19.456,11.776,35.84,19.968,46.592,25.088c-4.608,3.072-7.68,7.68-8.704,13.824c-1.024,5.632,0,11.776,3.072,16.896c3.072,5.12,8.192,8.192,13.824,9.728c1.536,0.512,3.072,0.512,4.608,0.512c4.096,0,8.704-1.024,12.288-3.584c5.12-3.072,8.192-8.192,9.728-13.824c1.024-5.632,0-11.776-3.072-16.896c-3.072-4.608-7.68-7.68-13.312-9.216c-0.512,0-22.016-8.192-52.224-26.112c2.56-0.512,5.632-1.024,8.192-1.024s5.632,0.512,8.192,1.024l77.824,15.872c22.528,4.608,36.864,26.624,32.256,48.64L255.09,382.692z M199.282,185.572c1.024-5.632,6.144-9.728,11.776-9.728c1.024,0,1.536,0,2.56,0c3.072,0.512,5.632,2.56,7.68,5.12c1.536,2.56,2.56,5.632,1.536,8.704c-0.512,3.072-2.56,5.632-5.12,7.68c-2.56,1.536-5.632,2.56-8.704,1.536c-3.072-0.512-5.632-2.56-7.68-5.12C199.282,191.716,198.77,188.644,199.282,185.572zM311.922,177.892l12.8,167.936c1.536,20.992-14.336,39.424-35.328,40.96l-27.136,2.048c0.512-2.048,1.024-3.584,1.536-5.632l12.8-62.976l1.536,25.6c0,2.56,2.56,4.608,5.12,4.608c0,0,0,0,0.512,0c3.072,0,5.12-2.56,4.608-5.632l-4.096-62.976l15.36-75.264c5.632-27.648-12.288-55.296-39.936-60.928l-8.704-1.536l19.456-1.536C291.954,141.028,310.386,156.9,311.922,177.892z"/>
                  </g>
                </g>
              </svg>
            </div>
            <div v-else>
              <svg class="Zi Zi--Live" fill="currentColor" style="color: rgb(67, 212, 128);" viewBox="0 0 512 512"
                   width="24" height="24">
                <g>
                  <path
                    d="M467,331h-15v-15c0-24.813-20.187-45-45-45H271v-30c14.095,0,30.95,0,45,0c8.284,0,15-6.716,15-15s-6.716-15-15-15h-15v-30h45c24.813,0,45-20.187,45-45V45c0-24.813-20.187-45-45-45H166c-24.813,0-45,20.187-45,45v91c0,24.813,20.187,45,45,45h45v30h-15c-8.284,0-15,6.716-15,15s6.716,15,15,15c14.05,0,30.905,0,45,0v30H105c-24.813,0-45,20.187-45,45v15H45c-24.813,0-45,20.187-45,45v30c0,24.813,20.187,45,45,45h15v31H45c-8.284,0-15,6.716-15,15s6.716,15,15,15h60c8.284,0,15-6.716,15-15s-6.716-15-15-15H90v-31h15c24.813,0,45-20.187,45-45v-30c0-24.813-20.187-45-45-45H90v-15c0-8.271,6.729-15,15-15h136v30h-15c-24.813,0-45,20.187-45,45v30c0,24.813,20.187,45,45,45h15v31h-15c-8.284,0-15,6.716-15,15s6.716,15,15,15h60c8.284,0,15-6.716,15-15s-6.716-15-15-15h-15v-31h15c24.813,0,45-20.187,45-45v-30c0-24.813-20.187-45-45-45h-15v-30h136c8.271,0,15,6.729,15,15v15h-15c-24.813,0-45,20.187-45,45v30c0,24.813,20.187,45,45,45h15v31h-15c-8.284,0-15,6.716-15,15s6.716,15,15,15h60c8.284,0,15-6.716,15-15s-6.716-15-15-15h-15v-31h15c24.813,0,45-20.187,45-45v-30C512,351.187,491.813,331,467,331z M105,361c8.271,0,15,6.729,15,15v30c0,8.271-6.729,15-15,15H45c-8.271,0-15-6.729-15-15v-30c0-8.271,6.729-15,15-15H105z M166,151c-8.271,0-15-6.729-15-15V45c0-8.271,6.729-15,15-15h180c8.271,0,15,6.729,15,15v91c0,8.271-6.729,15-15,15C309.96,151,201.911,151,166,151z M286,361c8.271,0,15,6.729,15,15v30c0,8.271-6.729,15-15,15h-60c-8.271,0-15-6.729-15-15v-30c0-8.271,6.729-15,15-15H286z M241,211v-30h30v30H241z M482,406c0,8.271-6.729,15-15,15h-60c-8.271,0-15-6.729-15-15v-30c0-8.271,6.729-15,15-15h60c8.271,0,15,6.729,15,15V406z"/>
                </g>
              </svg>
            </div>

            &nbsp {{displayMeta}}
          </a>
          <span class="TagsHeader">
                                # {{displayType=='tag'?'标签':'分类'}} #
                            </span>
        </nav>
      </div>
    </div>
    <div class="Card TopstoryItem TopstoryItem-isRecommend" v-for="content in data.list">
      <div class="Feed">
        <div class="ContentItem AnswerItem">
          <h2 class="ContentItem-title">
            <div itemprop="Blogs:question" itemtype="http://schema.org/Question" itemscope="">
              <a data-za-detail-view-element_name="Title" data-za-detail-view-id="2812"
                 :href="'/article/'+(content.slug&&content.slug!=null?encodeURIComponent(content.slug):content.id)">{{content.title}}</a>
            </div>
          </h2>
          <div class="RichContent is-collapsed">
            <div class="RichContent-inner">
                                                            <span class="RichText ztext CopyrightRichText-richText"
                                                                  itemprop="text">
                                                                {{content.content}}
                                                            </span>
              <a class="Button ContentItem-more Button--plain"
                 :href="'/article/'+(content.slug&&content.slug!=null?encodeURIComponent(content.slug):content.id)">
                阅读全文
              </a>
            </div>
            <div class="ContentItem-actions">
              <a class="Button ContentItem-action Button--plain Button--withIcon Button--withLabel"
                 :href="'/article/'+(content.slug&&content.slug!=null?encodeURIComponent(content.slug):content.id)+'#comment'">
                                                                <span
                                                                  style="display: inline-flex; align-items: center;">
                                                                    <svg class="Zi Zi--Comment Button-zi"
                                                                         fill="currentColor" width="1.2em"
                                                                         height="1.2em" viewBox="0 0 60 60">
                                                                        <g><path
                                                                          d="M10,25.465h13c0.553,0,1-0.448,1-1s-0.447-1-1-1H10c-0.553,0-1,0.448-1,1S9.447,25.465,10,25.465z"/><path
                                                                          d="M36,29.465H10c-0.553,0-1,0.448-1,1s0.447,1,1,1h26c0.553,0,1-0.448,1-1S36.553,29.465,36,29.465z"/><path
                                                                          d="M36,35.465H10c-0.553,0-1,0.448-1,1s0.447,1,1,1h26c0.553,0,1-0.448,1-1S36.553,35.465,36,35.465z"/><path
                                                                          d="M54.072,2.535L19.93,2.465c-3.27,0-5.93,2.66-5.93,5.93v5.124l-8.07,0.017c-3.27,0-5.93,2.66-5.93,5.93v21.141c0,3.27,2.66,5.929,5.93,5.929H12v10c0,0.413,0.254,0.784,0.64,0.933c0.117,0.045,0.239,0.067,0.36,0.067c0.276,0,0.547-0.115,0.74-0.327l9.704-10.675l16.626-0.068c3.27,0,5.93-2.66,5.93-5.929v-0.113l5.26,5.786c0.193,0.212,0.464,0.327,0.74,0.327c0.121,0,0.243-0.022,0.36-0.067c0.386-0.149,0.64-0.52,0.64-0.933v-10h1.07c3.27,0,5.93-2.66,5.93-5.929V8.465C60,5.196,57.341,2.536,54.072,2.535z M44,40.536c0,2.167-1.763,3.929-3.934,3.929l-17.07,0.07c-0.28,0.001-0.548,0.12-0.736,0.327L14,53.949v-8.414c0-0.552-0.447-1-1-1H5.93c-2.167,0-3.93-1.763-3.93-3.929V19.465c0-2.167,1.763-3.93,3.932-3.93l9.068-0.019c0,0,0,0,0,0c0.001,0,0.001,0,0.002,0l25.068-0.052c2.167,0,3.93,1.763,3.93,3.93v18.441V40.536z M58,29.606c0,2.167-1.763,3.929-3.93,3.929H52c-0.553,0-1,0.448-1,1v8.414l-5-5.5V19.395c0-3.27-2.66-5.93-5.932-5.93L16,13.514v-5.12c0-2.167,1.763-3.93,3.928-3.93l34.141,0.07c0.001,0,0.001,0,0.002,0c2.167,0,3.93,1.763,3.93,3.93V29.606z"/></g>
                                                                    </svg>评论({{content.commentsNum}})
                                                                </span>
              </a>
              <a class="Button ContentItem-action Button--plain Button--withIcon Button--withLabel"
                 :href="'/article/'+(content.slug&&content.slug!=null?encodeURIComponent(content.slug):content.id)">
                                                                <span
                                                                  style="display: inline-flex; align-items: center;">
                                                                    <svg class="Zi Zi--Heart Button-zi"
                                                                         fill="currentColor" width="1.2em"
                                                                         height="1.2em" viewBox="0 0 412.72 412.72">
                                                                        <path
                                                                          d="M404.72,82.944c-0.027,0-0.054,0-0.08,0h0h-27.12v-9.28c0.146-3.673-2.23-6.974-5.76-8c-18.828-4.934-38.216-7.408-57.68-7.36c-32,0-75.6,7.2-107.84,40c-32-33.12-75.92-40-107.84-40c-19.464-0.048-38.852,2.426-57.68,7.36c-3.53,1.026-5.906,4.327-5.76,8v9.2H8c-4.418,0-8,3.582-8,8v255.52c0,4.418,3.582,8,8,8c1.374-0.004,2.724-0.362,3.92-1.04c0.8-0.4,80.8-44.16,192.48-16h1.2h0.72c0.638,0.077,1.282,0.077,1.92,0c112-28.4,192,15.28,192.48,16c2.475,1.429,5.525,1.429,8,0c2.46-1.42,3.983-4.039,4-6.88V90.944C412.72,86.526,409.139,82.944,404.72,82.944z M16,333.664V98.944h19.12v200.64c-0.05,4.418,3.491,8.04,7.909,8.09c0.432,0.005,0.864-0.025,1.291-0.09c16.55-2.527,33.259-3.864,50-4c23.19-0.402,46.283,3.086,68.32,10.32C112.875,307.886,62.397,314.688,16,333.664z M94.32,287.664c-14.551,0.033-29.085,0.968-43.52,2.8V79.984c15.576-3.47,31.482-5.241,47.44-5.28c29.92,0,71.2,6.88,99.84,39.2l0.24,199.28C181.68,302.304,149.2,287.664,94.32,287.664zM214.32,113.904c28.64-32,69.92-39.2,99.84-39.2c15.957,0.047,31.863,1.817,47.44,5.28v210.48c-14.354-1.849-28.808-2.811-43.28-2.88c-54.56,0-87.12,14.64-104,25.52V113.904z M396.64,333.664c-46.496-19.028-97.09-25.831-146.96-19.76c22.141-7.26,45.344-10.749,68.64-10.32c16.846,0.094,33.663,1.404,50.32,3.92c4.368,0.663,8.447-2.341,9.11-6.709c0.065-0.427,0.095-0.859,0.09-1.291V98.944h19.12L396.64,333.664z"/>
                                                                    </svg>阅读({{content.hits}})
                                                                </span>
              </a>

              <a class="ContentItem-action Button Button--plain Button--withIcon Button--withLabel" href="archives">
                                                                <span
                                                                  style="display: inline-flex; align-items: center;">
                                                                    <svg class="Zi Zi--Share Button-zi"
                                                                         fill="currentColor" width="1.2em"
                                                                         height="1.2em" viewBox="0 0 203.543 203.543">
                                                                        <g><path
                                                                          d="m194.139,11.918h-39.315v-5.918c0-3.313-2.687-6-6-6s-6,2.687-6,6v5.918h-35.053v-5.918c0-3.313-2.687-6-6-6s-6,2.687-6,6v5.918h-33.563v-5.918c0-3.313-2.687-6-6-6s-6,2.687-6,6v5.918h-40.804c-3.313,0-6,2.687-6,6v135.572c0,3.313 2.687,6 6,6h36.826c5.901,25.214 28.555,44.053 55.541,44.053s49.64-18.84 55.541-44.053h36.826c3.313,0 6-2.687 6-6v-135.572c0.001-3.313-2.685-6-5.999-6zm-143.931,12v5.422c0,3.313 2.687,6 6,6s6-2.687 6-6v-5.422h33.563v5.422c0,3.313 2.687,6 6,6s6-2.687 6-6v-5.422h35.053v5.422c0,3.313 2.687,6 6,6s6-2.687 6-6v-5.422h33.315v24.536h-172.735v-24.536h34.804zm51.563,167.625c-24.842,0-45.053-20.211-45.053-45.053s20.211-45.053 45.053-45.053 45.053,20.21 45.053,45.053-20.21,45.053-45.053,45.053zm57.028-44.053c0.006-0.334 0.025-0.665 0.025-1 0-31.459-25.594-57.053-57.053-57.053s-57.053,25.594-57.053,57.053c0,0.335 0.02,0.666 0.025,1h-29.34v-87.035h172.735v87.035h-29.339z"/><path
                                                                          d="m107.771,148.004v-29.026c0-3.313-2.687-6-6-6s-6,2.687-6,6v31.512c0,1.591 0.632,3.117 1.757,4.243l13.79,13.791c1.172,1.171 2.707,1.757 4.243,1.757 1.535,0 3.071-0.586 4.243-1.757 2.343-2.343 2.343-6.142 0-8.485l-12.033-12.035z"/></g>
                                                                    </svg>
                                                                    {{content.created | formatDate}}
                                                                </span>
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="lists-navigator clearfix" v-if="displayType=='index'">
      <ol class="page-navigator">
        <li class="prev" v-if="data.pageNum>1"><a :href="'/index/'+(data.pageNum-1)">←</a></li>
        <li :class="index==data.pageNum?'current':''" v-for="index in getPageNums"><a
          :href="'/index/'+index">{{index}}</a>
        </li>
        <li class="next" v-if="data.pageNum<data.totalPage"><a :href="'/index/'+(data.pageNum+1)">→</a></li>
      </ol>
    </div>
    <div class="lists-navigator clearfix" v-else>
      <ol class="page-navigator">
        <li class="prev" v-if="data.pageNum>1"><a
          :href="'/'+displayType+'/'+encodeURIComponent(displayMeta)+'/'+(data.pageNum-1)">←</a></li>
        <li :class="index==data.pageNum?'current':''" v-for="index in getPageNums"><a
          :href="'/'+displayType+'/'+encodeURIComponent(displayMeta)+'/'+index">{{index}}</a>
        </li>
        <li class="next" v-if="data.pageNum<data.totalPage"><a
          :href="'/'+displayType+'/'+encodeURIComponent(displayMeta)+'/'+(data.pageNum+1)">→</a></li>
      </ol>
    </div>
  </div>
</template>
<script>

  import {dateFormat} from "../../../utils/dateUtils";

  export default {
    name: 'Content',
    props: ['displayType', 'displayMeta', 'orderType', 'data'],
    filters: {
      formatDate(time) {
        const date = new Date(time);
        return dateFormat(date, 'yyyy-MM-dd');
      }
    },
    computed: {
      getPageNums: function () {
        let startIndex = this.data.pageNum - 2;
        if (startIndex < 1) {
          startIndex = 1;
        }
        let endIndex = this.data.pageNum + 2;
        if (endIndex - startIndex < 4) {
          endIndex = startIndex + 4;
        }
        if (endIndex > this.data.totalPage) {
          endIndex = this.data.totalPage;
        }
        let nums = [1];
        for (let i = startIndex; i <= endIndex; i++) {
          if (i !== 1 && i !== this.data.totalPage) {
            nums.push(i)
          }
        }
        nums.push(this.data.totalPage);
        return nums;
      }
    }
  }
</script>
<style>

</style>
